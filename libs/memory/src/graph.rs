...
use kernel::agent_profile::{get_agent_profile, sanitize_component};
...
impl CognitiveGraph {
    pub async fn project_social_tree(&self, user_id: &str, memory_hint: f32) -> Result<SocialTreeSnapshot> {
        ...
        let _existing_updated_at = extract_string(&mut response, 12);
        ...
        let persisted_snapshot = SocialTreeSnapshot {
        ...
        let content = serde_json::json!({
            ...
            "relationship_core": persisted_snapshot.relationship_core,
            "dynamic_state": persisted_snapshot.dynamic_state,
            "self_other_model": persisted_snapshot.self_other_model,
            "derived_summaries": persisted_snapshot.derived_summaries,
            "meta": persisted_snapshot.meta,
        });

        self.db
            .query(&format!("UPSERT social_tree_root:`{}` CONTENT $content;", safe_user_id))
            .bind(("content", content))
            .await
            .context("Failed to UPSERT social tree root")?
            .check()
            .context("Failed to persist social tree root")?;
    ...
    pub async fn update_observed_dynamic(&self, from_user: &str, to_user: &str, tension: f32) -> Result<()> {
        let from_user = sanitize_component(from_user);
        let to_user = sanitize_component(to_user);
        let edge_id = format!("{}_{}", from_user, to_user);
        let query = format!(
            r#"
            UPSERT interacts_with:`{}` CONTENT {{
                in: person:`{}`,
                out: person:`{}`,
                tension: math::clamp((interacts_with:`{}`.tension OR 0.0) + $tension, -1.0, 1.0)
            }};
        "#,
            edge_id, from_user, to_user, edge_id
        );
