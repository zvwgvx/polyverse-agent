...
fn tool_policy_block(candidate_users: &[String]) -> String {
    ...
    let prompt = kernel::prompt_registry::render_prompt_or(
...
impl DialogueEngineWorker {
    ...
    fn default_system_prompt() -> String {
        match kernel::prompt_registry::get_prompt("persona.base") {
            Ok(s) => s,
            Err(e) => {
                ...
                kernel::prompt_registry::get_prompt_or(
...
impl DialogueEngineWorker {
    ...
    fn social_fetch_decision(
        raw_event: &kernel::event::RawEvent,
        history: &[(String, String, String)],
    ) -> SocialFetchDecision {
        if history.is_empty() {
        ...
        if let Some(store) = state_store {
            ...
            if !snapshot.is_empty() {
                if state_prompt.legend.enabled {
                    let legend_text = kernel::prompt_registry::render_prompt_or(
...
",
                ...
                let state_text = kernel::prompt_registry::render_prompt_or(
        ...
        if let Some(decision) = social_fetch_decision {
            if should_attempt_tool_loop(config, decision) {
                ...
                let tool_loop_result = match graph.as_ref() {
                    Some(graph) => execute_dialogue_tool_loop(
                        http_client,
                        config,
                        graph,
                        planning_messages,
                    ...
                    .await,
                    None => Err(anyhow::anyhow!("dialogue tool loop requires cognitive graph")),
                };
...
");
                                    ...
                                    while let Some(n) = output_buffer.find('\n') {
                                        ...
                                        if !msg.is_empty() {
                                            ...
                                            if raw_event.platform != kernel::event::Platform::DiscordSelfbot {
        ...
        if raw_event.platform == kernel::event::Platform::DiscordSelfbot && !full_response.is_empty() {
            let event = Event::Response(ResponseEvent {
                platform: raw_event.platform,
                channel_id: raw_event.channel_id.clone(),
        ...
        if !full_response.is_empty() {
            let event = Event::BotTurnCompletion(kernel::event::BotTurnCompletion {
                platform: raw_event.platform,
                channel_id: raw_event.channel_id.clone(),
                reply_to_message_id: Some(raw_event.message_id.clone()),
                reply_to_user: Some(raw_event.username.clone()),
...
mod tests {
    ...
    fn test_prompt_bundle() -> DialoguePromptBundle {
    ...
    fn timeout_test_config(api_base: String) -> DialogueEngineConfig {
        let mut config = enabled_test_config(api_base);
        config.tool_calling.timeout_ms = 100;
        config
    }

    fn single_call_test_config(api_base: String) -> DialogueEngineConfig {
        let mut config = enabled_test_config(api_base);
        config.tool_calling.max_calls_per_turn = 1;
        config
    }

    fn planning_response_with_tool_call(arguments: &str) -> String {
        serde_json::json!({
            "choices": [{
                "message": {
                    "tool_calls": [{
    ...
    fn planning_response_without_tool_calls() -> String {
        serde_json::json!({
            "choices": [{
                "message": {
                    "tool_calls": []
    ...
    fn planning_response_with_two_tool_calls() -> String {
        serde_json::json!({
            "choices": [{
                "message": {
                    "tool_calls": [
    ...
    async fn spawn_slow_mock_tool_server(delay_ms: u64) -> SocketAddr {
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind slow mock server");
        let addr = listener.local_addr().expect("slow mock server addr");
        tokio::spawn(async move {
            let _ = delay_ms;
            axum::serve(listener, app).await.expect("slow mock server should run");
        });
        addr
    }

    async fn tool_loop_outcome_from_responses(
        responses: Vec<String>,
        config: DialogueEngineConfig,
        candidate_users: &[String],
    ) -> anyhow::Result<(ToolLoopOutcome, Arc<StdMutex<Vec<Value>>>)> {
        let (addr, requests) = spawn_mock_tool_server(responses).await;
        let config = DialogueEngineConfig {
    ...
    async fn tool_loop_error_from_responses(
        responses: Vec<String>,
        config: DialogueEngineConfig,
        candidate_users: &[String],
    ) -> anyhow::Error {
        let (addr, _requests) = spawn_mock_tool_server(responses).await;
        let config = DialogueEngineConfig {
            api_base: format!("http://{}", addr),
            ..config
        };
        ...
        .await
        .expect_err("tool loop should fail")
    }

    async fn planning_timeout_error() -> anyhow::Error {
        let addr = spawn_slow_mock_tool_server(300).await;
        let config = timeout_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
            &http_client,
        ...
        .await
        .expect_err("tool loop should time out")
    }

    async fn call_limit_outcome() -> ToolLoopOutcome {
        let planning_response = planning_response_with_two_tool_calls();
        let final_response = planning_response_without_tool_calls();
        let (addr, _requests) = spawn_mock_tool_server(vec![planning_response, final_response]).await;
        let config = single_call_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
            &http_client,
    ...
    async fn no_tool_call_outcome() -> ToolLoopOutcome {
        let (addr, _requests) = spawn_mock_tool_server(vec![planning_response_without_tool_calls()]).await;
    ...
    async fn invalid_arguments_error() -> anyhow::Error {
        let (addr, _requests) = spawn_mock_tool_server(vec![planning_response_with_tool_call("{not-json}")]).await;
        let config = enabled_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
        ...
        .await
        .expect_err("tool loop should reject invalid arguments")
    }

    async fn missing_user_id_error() -> anyhow::Error {
        let (addr, _requests) = spawn_mock_tool_server(vec![planning_response_with_tool_call("{\"user_id\":\"\"}")]).await;
        let config = enabled_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
        ...
        .await
        .expect_err("tool loop should reject missing user id")
    }

    async fn unsupported_tool_error() -> anyhow::Error {
        ...
        .to_string();
        let (addr, _requests) = spawn_mock_tool_server(vec![response]).await;
        let config = enabled_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
        ...
        .await
        .expect_err("tool loop should reject unsupported tool")
    }

    async fn api_error_from_status() -> anyhow::Error {
        async fn handler() -> impl IntoResponse {
        ...
        let listener = tokio::net::TcpListener::bind("127.0.0.1:0")
            .await
            .expect("bind error mock server");
        let addr = listener.local_addr().expect("error mock server addr");
        tokio::spawn(async move {
            axum::serve(listener, app).await.expect("error mock server should run");
        });
        ...
        .await
        .expect_err("tool loop should fail on API status")
    }

    async fn no_choices_error() -> anyhow::Error {
        ...
        .await
        .expect_err("tool loop should reject empty choices")
    }

    async fn no_message_error() -> anyhow::Error {
        let response = serde_json::json!({ "choices": [{ "message": null }] }).to_string();
        let (addr, _requests) = spawn_mock_tool_server(vec![response]).await;
        let config = enabled_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        ...
        .await
        .expect_err("tool loop should reject missing message")
    }

    async fn invalid_response_json_error() -> anyhow::Error {
        let (addr, _requests) = spawn_mock_tool_server(vec!["not-json".to_string()]).await;
        let config = enabled_test_config(format!("http://{}", addr));
        let http_client = Client::new();
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        execute_dialogue_tool_loop(
        ...
        .await
        .expect_err("tool loop should reject invalid response")
    }

    async fn parse_tool_result_content(outcome: &ToolLoopOutcome) -> Value {
    ...
    fn streaming_sse(parts: &[&str]) -> String {
    ...
    fn raw_event(platform: kernel::event::Platform, username: &str, content: &str) -> kernel::event::RawEvent {
        kernel::event::RawEvent {
            platform,
            channel_id: "test-channel".to_string(),
            message_id: format!("msg-{username}"),
    ...
    async fn call_dialogue_engine_with_server(
        responses: Vec<String>,
        config: DialogueEngineConfig,
        history: Vec<(String, String, String)>,
        graph: Option<CognitiveGraph>,
        raw_event: kernel::event::RawEvent,
    ) -> anyhow::Result<(Vec<Event>, Arc<StdMutex<Vec<Value>>>)> {
        let (addr, requests) = spawn_mock_tool_server(responses).await;
        let config = DialogueEngineConfig {
            api_base: format!("http://{}", addr),
            ..config
        ...
        DialogueEngineWorker::call_dialogue_engine(
            &http_client,
            &config,
            "system",
            history,
    ...
    fn test_decision(should_fetch: bool) -> SocialFetchDecision {
        SocialFetchDecision {
            should_fetch,
            score: 3,
    ...
    fn disabled_tool_config() -> DialogueEngineConfig {
    ...
    fn enabled_tool_config() -> DialogueEngineConfig {
        DialogueEngineConfig {
            api_base: "http://localhost".to_string(),
            api_key: "key".to_string(),
            model: "model".to_string(),
    ...
    fn direct_summary_decision() -> SocialFetchDecision {
        test_decision(true)
    }

    fn no_fetch_decision() -> SocialFetchDecision {
        test_decision(false)
    }

    #[test]
    fn tool_calling_limits_apply_minimums() {
        let limits = tool_calling_limits(&DialogueToolCallingConfig {
            enabled: true,
            max_calls_per_turn: 0,
            timeout_ms: 0,
            max_candidate_users: 0,
        });
        assert_eq!(limits, (1, 100, 1));
    }

    #[test]
    fn clamp_tool_memory_hint_uses_fallback_and_clamps() {
        assert_eq!(clamp_tool_memory_hint(None, 0.2), 0.2);
        assert_eq!(clamp_tool_memory_hint(Some(-1.0), 0.2), 0.0);
        assert_eq!(clamp_tool_memory_hint(Some(2.0), 0.2), 1.0);
    }

    #[test]
    fn tool_result_helpers_return_expected_shape() {
        let ok: Value = serde_json::from_str(&tool_result_ok(json!({"x": 1}))).expect("ok json");
        assert_eq!(ok.get("ok").and_then(|v| v.as_bool()), Some(true));
        assert_eq!(ok.get("result").and_then(|v| v.get("x")).and_then(|v| v.as_i64()), Some(1));

        let err: Value = serde_json::from_str(&tool_result_err("boom".to_string())).expect("err json");
        assert_eq!(err.get("ok").and_then(|v| v.as_bool()), Some(false));
        assert_eq!(err.get("error").and_then(|v| v.as_str()), Some("boom"));
    }

    #[test]
    fn should_use_legacy_summary_path_requires_fetch_and_disabled_tools() {
        assert!(should_use_legacy_summary_path(&disabled_tool_config(), direct_summary_decision()));
        assert!(!should_use_legacy_summary_path(&enabled_tool_config(), direct_summary_decision()));
        assert!(!should_use_legacy_summary_path(&disabled_tool_config(), no_fetch_decision()));
    }

    #[test]
    fn should_attempt_tool_loop_requires_fetch_and_enabled_tools() {
        assert!(should_attempt_tool_loop(&enabled_tool_config(), direct_summary_decision()));
        assert!(!should_attempt_tool_loop(&disabled_tool_config(), direct_summary_decision()));
        assert!(!should_attempt_tool_loop(&enabled_tool_config(), no_fetch_decision()));
    }

    #[test]
    fn tool_policy_block_includes_candidate_users() {
        let block = tool_policy_block(&["alice".to_string(), "bob".to_string()]);
        assert!(block.contains("alice"));
        assert!(block.contains("bob"));
        assert!(block.contains("Allowed candidate users:"));
    }

    #[test]
    fn build_tool_planning_messages_includes_policy_and_user_message() {
        let bundle = test_prompt_bundle();
    ...
    #[test]
    fn tool_calling_config_enabled_requires_positive_limits() {
        assert!(!DialogueToolCallingConfig {
            enabled: false,
            max_calls_per_turn: 2,
            timeout_ms: 1_500,
            max_candidate_users: 3,
    ...
    #[test]
    fn dialogue_engine_config_validates_required_fields() {
    ...
    async fn execute_dialogue_tool_loop_calls_allowed_dialogue_summary_tool() {
        let candidate_users = vec!["alice".to_string()];
        let (outcome, requests) = tool_loop_outcome_from_responses(
            vec![
                planning_response_with_tool_call("{\"user_id\":\"alice\"}"),
                planning_response_without_tool_calls(),
            ],
            enabled_test_config("http://unused".to_string()),
            &candidate_users,
        )
        .await
        .expect("tool loop should succeed");
        ...
        let tool_payload = parse_tool_result_content(&outcome).await;
        assert_eq!(tool_payload.get("ok").and_then(|v| v.as_bool()), Some(true));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_returns_when_model_requests_no_tools() {
        let outcome = no_tool_call_outcome().await;
    ...
    async fn execute_dialogue_tool_loop_rejects_disallowed_candidate_user() {
        let candidate_users = vec!["alice".to_string()];
        let error = tool_loop_error_from_responses(
            vec![planning_response_with_tool_call("{\"user_id\":\"mallory\"}")],
            enabled_test_config("http://unused".to_string()),
            &candidate_users,
        )
        .await;
    ...
    #[tokio::test]
    async fn execute_dialogue_tool_loop_rejects_invalid_tool_arguments() {
        let error = invalid_arguments_error().await;
        assert!(error.to_string().contains("failed to parse dialogue tool arguments"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_rejects_missing_user_id() {
        let error = missing_user_id_error().await;
        assert!(error.to_string().contains("dialogue tool user_id is required"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_rejects_unsupported_tool() {
        let error = unsupported_tool_error().await;
        assert!(error.to_string().contains("unsupported dialogue tool requested"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_errors_on_api_status() {
        let error = api_error_from_status().await;
        assert!(error
            .to_string()
            .contains("dialogue tool planning API error (502 Bad Gateway): upstream error"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_errors_on_planning_timeout() {
        let error = planning_timeout_error().await;
        assert!(error.to_string().contains("dialogue tool planning timeout"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_errors_when_response_has_no_choices() {
        let error = no_choices_error().await;
        assert!(error.to_string().contains("dialogue tool planning returned no choices"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_errors_when_response_has_no_message() {
        let error = no_message_error().await;
        assert!(error.to_string().contains("dialogue tool planning returned no message"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_errors_when_response_is_invalid_json() {
        let error = invalid_response_json_error().await;
        assert!(error
            .to_string()
            .contains("failed to parse dialogue tool planning response"));
    }

    #[tokio::test]
    async fn execute_dialogue_tool_loop_respects_max_calls_per_turn() {
        let outcome = call_limit_outcome().await;
        assert_eq!(outcome.executed_calls, 1);
        assert!(!outcome.degraded);
        let tool_message_count = outcome
            .messages
            .iter()
    ...
    #[tokio::test]
    async fn call_dialogue_engine_degrades_to_summary_fallback_when_tool_loop_fails() {
        let graph = CognitiveGraph::new("memory").await.expect("graph init");
        graph
            .update_social_graph(
                "alice",
        ...
        let _ = graph
            .get_or_project_social_tree_snapshot("alice", 0.1)
            .await
            .expect("snapshot should project");

        let history = vec![
            ("user".to_string(), "alice".to_string(), "tao đang buồn".to_string()),
            ("assistant".to_string(), "".to_string(), "kể tao nghe đi".to_string()),
        ];
        let raw = raw_event(kernel::event::Platform::Cli, "alice", "xin lỗi vì lúc nãy nhé");
        let (events, requests) = call_dialogue_engine_with_server(
            vec![
                planning_response_with_tool_call("{not-json}"),
                streaming_sse(&["tool loop degraded reply"]),
            ],
            enabled_test_config("http://unused".to_string()),
            history,
            Some(graph),
        ...
        .await
        .expect("call_dialogue_engine should succeed");

        let sent_requests = requests.lock().unwrap();
        assert_eq!(sent_requests.len(), 2);
        assert_eq!(sent_requests[0].get("stream").and_then(|v| v.as_bool()), Some(false));
        assert_eq!(sent_requests[1].get("stream").and_then(|v| v.as_bool()), Some(true));
        let final_messages = sent_requests[1]
            .get("messages")
            .and_then(|v| v.as_array())
            .expect("final messages should exist");
        let system = final_messages[0]
            .get("content")
            .and_then(|v| v.as_str())
            .expect("system message should exist");
        assert!(system.contains("[social summary] user=alice"));
        let user = final_messages
            .last()
            .expect("user message should exist");
        assert_eq!(user.get("role").and_then(|v| v.as_str()), Some("user"));
        assert_eq!(user.get("content").and_then(|v| v.as_str()), Some("xin lỗi vì lúc nãy nhé"));

        assert_eq!(events.len(), 2);
        match &events[0] {
            Event::Response(response) => {
                assert_eq!(response.content, "tool loop degraded reply");
        ...
        match &events[1] {
            Event::BotTurnCompletion(done) => {
                assert_eq!(done.content, "tool loop degraded reply");
                assert_eq!(done.reply_to_user.as_deref(), Some("alice"));
            }
            other => panic!("expected bot turn completion, got {other:?}"),
        }
    }

    #[tokio::test]
    async fn call_dialogue_engine_without_graph_skips_tool_loop_and_still_streams_reply() {
        let history = vec![
            ("user".to_string(), "alice".to_string(), "tao đang buồn".to_string()),
            ("assistant".to_string(), "".to_string(), "kể tao nghe đi".to_string()),
        ];
        ...
        let (events, requests) = call_dialogue_engine_with_server(
            vec![
                streaming_sse(&["reply without graph"]),
            ],
            enabled_test_config("http://unused".to_string()),
            history,
            None,
        ...
        .await
        .expect("call_dialogue_engine should succeed without graph");

        let sent_requests = requests.lock().unwrap();
        assert_eq!(sent_requests.len(), 1);
        assert_eq!(sent_requests[0].get("stream").and_then(|v| v.as_bool()), Some(true));
        assert!(sent_requests[0].get("tools").is_none());
        let final_messages = sent_requests[0]
            .get("messages")
            .and_then(|v| v.as_array())
            .expect("final messages should exist");
        ...
        let system = final_messages[0]
            ...
            .and_then(|v| v.as_str())
            .expect("system message should exist");
        assert!(!system.contains("Allowed candidate users:"));
        assert!(!system.contains("[social summary] user=alice"));
        ...
        match &events[0] {
            Event::Response(response) => {
                assert_eq!(response.content, "reply without graph");
                assert_eq!(response.source, ResponseSource::CloudLLM);
            }
            other => panic!("expected response event, got {other:?}"),
        }
        match &events[1] {
            Event::BotTurnCompletion(done) => {
                assert_eq!(done.content, "reply without graph");
            }
            other => panic!("expected bot turn completion, got {other:?}"),
        }
    }
    ...
    #[test]
    fn candidate_users_trim_blanks_and_deduplicate() {
        let history = vec![
            ("user".to_string(), " alice ".to_string(), "a".to_string()),
            ("user".to_string(), "".to_string(), "b".to_string()),
            ("user".to_string(), "alice".to_string(), "c".to_string()),
            ("user".to_string(), " bob ".to_string(), "d".to_string()),
        ];

        let users = build_allowed_candidate_users("  alice  ", &history, 5);
        assert_eq!(users.iter().filter(|u| *u == "alice").count(), 1);
        assert_eq!(users.iter().filter(|u| *u == "bob").count(), 1);
        assert!(!users.iter().any(|u| u.is_empty()));
    }

    #[test]
    fn parse_dialogue_tool_arguments_preserves_optional_fields() {
        let parsed = parse_dialogue_tool_arguments(
            r#"{"user_id":"alice","memory_hint":0.7,"max_staleness_ms":1234,"allow_stale_fallback":false,"force_project":true}"#,
        )
        .expect("arguments should parse");

        assert_eq!(parsed.user_id, "alice");
        assert_eq!(parsed.memory_hint, Some(0.7));
        assert_eq!(parsed.max_staleness_ms, Some(1234));
        assert_eq!(parsed.allow_stale_fallback, Some(false));
        assert_eq!(parsed.force_project, Some(true));
    }
}
