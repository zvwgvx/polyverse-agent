...
- Run MCP tests: `cargo test -p pa-mcp`
...
- `crates/pa-core`: shared contracts, event types, worker traits, prompt registry, and agent profile loading
- `crates/pa-runtime`: `Supervisor`, `EventBus`, and `Coordinator`
- `crates/pa-sensory`: platform adapters for Discord bot, Discord selfbot websocket, and Telegram
- `crates/pa-cognitive`: dialogue engine, affect evaluator, social-query facade, and dialogue tool registry
- `crates/pa-memory`: persistence, short-term memory, episodic memory, semantic compression, and cognitive graph
- `crates/pa-state`: state schema/store and state-derivation workers
- `crates/pa-cockpit-api`: local Axum API for observability and prompt/state tooling
- `crates/pa-mcp`: local read-only MCP transport and tool registry wrapper
- `crates/pa-agent`: composition root / main binary
...
- Runtime config is layered in `crates/pa-agent/src/main.rs` like this:
  1. `.env` via `dotenvy`
...
- `settings.json` is read from the repo root by `pa-agent`; it does not replace API credentials, but it can quietly override local runtime behavior.
...
- Important checked-in config files:
  ...
  - `config/state_schema.v0.json` defines the state dimensions loaded by `pa-state`
...
This repo is a worker-based agent runtime. `pa-agent` wires the system together and conditionally registers workers based on env/config.
...
- `pa-core` holds the shared contract for the whole system:
  ...
  - agent profile loading in `crates/pa-core/src/agent_profile.rs`
  - prompt registry helpers in `crates/pa-core/src/prompt_registry.rs`
- The current runtime wiring in `pa-agent` can register:
...
- `pa-cognitive`: LLM-facing workers and social query logic.
...
- `pa-cockpit-api`: local Axum server exposing observability/debug APIs over worker status, recent events, state rows/history/metrics, memory, episodic memory, relationship graph snapshots, system metrics, and prompt documents. It also supports prompt reads/updates and manual state patching.
- `pa-mcp`: local Axum worker exposing read-only MCP-style endpoints:
  - `GET /api/mcp/tools`
  - `POST /api/mcp/tools/call`
  It currently serves `social.get_affect_context` and `social.get_dialogue_summary` by delegating to `pa-cognitive`'s dialogue tool registry.
...
- Social retrieval is now intentionally split into write-vs-read layers:
  ...
  - `pa-cognitive::social_context` = tree-first query facade with `tree_fresh`, `tree_stale`, `graph_fallback`, and `default_fallback` source metadata
- The MCP registry in `pa-mcp` is intentionally thin. The actual tool definitions and execution logic live in `pa-cognitive::dialogue_tools`, so new read tools usually need changes there first, then transport/tests in `pa-mcp`.
